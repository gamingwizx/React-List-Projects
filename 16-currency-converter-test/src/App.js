  ...
  function convertCurrency(value) {
    ...
    async function convert() {
      const res = await fetch(
        `https://api.frankfurter.app/latest?amount=${value}&from=${conversionFrom}&to=${conversionTo}`,
    ...
    <div className="App">
      <div className="currency-converter">
        ...
        <CurrencyList
          ...
          onChange={(value) => setConversionFrom(value)}
        />
        <CurrencyList
          key="2"
          currencyList={currencyList}
          onChange={(value) => setConversionTo(value)}
        />
  ...
  return (
    <select onChange={(e) => onChange(e.target.value)}>
      {Object.keys(currencyList).map((key, i) => (
        <option key={key} defaultValue={key} value={key}>
          {key}
        </option>
      ))}
    </select>
